...
- **SSRF-защита** (найдено security-review, исправлено 2026-07-26): `source.URL` для `rss`/`youtube` — целиком пользовательский ввод, который `cmd/worker` затем запрашивает сам, с сервера; без защиты пользователь мог зарегистрировать источник вида `http://169.254.169.254/...` и прочитать ответ внутреннего эндпоинта через собственный же брифинг (контент попадает в `posts` → `events` → объяснение). Две независимые проверки, каждая закрывает свою часть поверхности:
  - `source.Service.AddSource` (`internal/source/service`) отклоняет схему, отличную от `http`/`https` (`domain.ErrUnsupportedURLScheme`, 400) — это только формат, не гарантия безопасности адреса: сразу после создания источника DNS ещё может указывать куда угодно.
  - `internal/source/fetcher/safeclient.go` — `RSSFetcher` использует `http.Client` с кастомным `DialContext`, который резолвит хост и отклоняет запрос, если резолвнутый IP — loopback/private/link-local/unspecified/multicast, **на каждый dial**, включая редиректы (`Transport` вызывает `DialContext` при каждом переходе, а не только на первый запрос) и без повторного резолвинга хоста в момент коннекта (соединение идёт на уже проверенный IP) — так закрывается и DNS rebinding между проверкой и запросом. Это единственный источник истины для проверки адреса; `AddSource` намеренно не дублирует список приватных диапазонов.
  - `TelegramFetcher` этой уязвимости не подвержен: `telegramPreviewURL` (Этап 6) игнорирует пользовательский хост и всегда обращается к захардкоженному `t.me`/`telegram.me`, поэтому не тронут.
