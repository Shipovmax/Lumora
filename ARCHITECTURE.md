...
| 12. Тестирование | `*_test.go` рядом с кодом в каждом домене | Unit (testify) + Integration (testcontainers-go) | частично (auth, user, usercontext, source, ingest) |
...
Этапы 1 (инициализация), 2 (авторизация), 3 (профиль пользователя), 4 (контекст пользователя), 5 (источники) и 6 (импорт данных) реализованы по одинаковой слоистой структуре домена. Этап 2 провалидирован end-to-end через `docker compose` (register/login/refresh-ротация/logout/me).
...
Следующий шаг — Этап 7 (обработка событий: `internal/pipeline`, очистка → дедупликация → кластеризация → определение темы → определение важности → формирование события; asynq-задача `pipeline:process`, потребляет `posts` из Этапа 6).
